...
def get_ascending_flat_matrix(rows, cols, rand=False):
  mat = np.zeros(rows * cols)
  for i in range(rows):
    for j in range(cols):
      if rand:
        mat[i*cols + j] = np.random.rand(1)
      else:
...
def test_conv():
  ...
  # build input matrix
  arr1 = get_ascending_flat_matrix(rows, cols,False)
  
  # mean filter
  mean_filter = np.array([0.5,0.5])
  ...
  # warmup the gpu
  w1 = cuda_processing.warmup(1)
  
  # move values to gpu
  arr1_ptr = cuda_processing.move_to_gpu(arr1)
  arr2_ptr = cuda_processing.move_to_gpu(arr2)
  mean_filter_ptr = cuda_processing.move_to_gpu(mean_filter)
  
  # perform convolution
  cuda_processing.direct_conv_wrap(arr1_ptr, arr2_ptr, mean_filter_ptr, mean_filter.shape[0], cols, rows, cols*rows)
  
  # move output matrix to cpu
  cuda_processing.move_to_cpu(arr2_ptr, arr3)
  
  #pretty print arrays
  arrprint(arr1, cols, rows)
  print(f"{'-'*10*cols}")
...
def minimize_test():
  ...
  # build input matrix
  # arr1 = get_ascending_flat_matrix(rows, cols,False)
  vals = np.random.rand(cols*rows)*100
  inc_vals = np.random.rand(2,cols*rows)*100
  ages = np.zeros(cols*rows)
  max_age = 5
  v1 = cuda_processing.warmup(1)
  vals_ptr = cuda_processing.move_to_gpu(vals)
  inc_vals_ptr = cuda_processing.move_to_gpu(inc_vals[0])
  ages_ptr = cuda_processing.move_to_gpu(ages)
  
  s1 = time.perf_counter()
  for i in range(1,inc_vals.shape[0]):
    cuda_processing.direct_minimize_wrap(vals_ptr, ages_ptr, inc_vals_ptr, max_age,cols, rows, vals.shape[0])
    cuda_processing.move_to_gpu_addr(inc_vals_ptr, inc_vals[i], inc_vals[i].shape[0])
  s2 = time.perf_counter()
  
  print(f"minimize:{s2-s1}")
  
  cuda_processing.move_to_gpu_addr(vals_ptr, vals, vals.shape[0])
  cuda_processing.move_to_gpu_addr(inc_vals_ptr, inc_vals[0],inc_vals[0].shape[0])
  cuda_processing.move_to_gpu_addr(ages_ptr, ages, ages.shape[0])
  s3 = time.perf_counter()
  for i in range(1, inc_vals.shape[0]):
    cuda_processing.sliding_window(vals_ptr, ages_ptr, inc_vals_ptr, max_age, vals.shape[0])
    cuda_processing.move_to_gpu_addr(inc_vals_ptr, inc_vals[i],inc_vals[i].shape[0])
  s4 = time.perf_counter()
  print(f"sliding:{s4-s3}")
