...
def main():
    ...
    lpf_np = np.array([0,0,1,1,1,1,0,0])
    ...
    with dai.Device(pp) as device:
        ...
        while not rospy.is_shutdown() and True:
            ...
            if DEPTH and depthFrame != None:
                ...
                # apply minimization
                cuda_processing.direct_minimize_wrap(rolling_frame_ptr, ages_ptr, new_vals_ptr,RATE, 640,400, 640*400)
                ...
                if DISPLAY and c==0:
                    ...
                    # apply mean filter
                    cuda_processing.direct_conv_wrap(conv_arr1_ptr, conv_arr2_ptr, mean_flt_ptr, mean_flt.shape[0], 640, 400, 400*640)
                    ...
                    # find markers
                    cuda_processing.find_min_wrap(conv_arr1_ptr, markers_gpu, rolling_frame_ptr,0.05 ,80, 640, 400)
                    cuda_processing.invert_wrap(markers_gpu,num, 640,1)
                    #apply pre warp
                    cuda_processing.prewarp_wrap(markers_gpu, markers_hold_gpu, 290, 580, theta_max, 29.5,640)
                    #cuda_processing.direct_conv_wrap(markers_gpu, markers_hold_gpu, lpf_gpu,lpf_np.shape[0],1,640,640)
                    ...
                    #    "raw",
                    #    cv2.hconcat(
                    #        [display_np.reshape(400,640)*0.01]
                    #    ),
                    #)
                    cv2.imshow(
                        "scatter", scatterplot((400,640), markers_hold[:, np.newaxis])
                    )
